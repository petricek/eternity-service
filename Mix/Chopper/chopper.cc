  ...
  :Runable(chopperLogFile), MixOffspring(parent), Shutdownable(mix->killer)
{
  ...
  string tmp;
  tmp = mix->configFile->GetValue(CFG_FIXED_CHUNK_SIZE);
  if (tmp.empty()) {
    fixedChunkSize = DEFAULT_FIXED_CHUNK_SIZE;
  }
  else {
    fixedChunkSize = (Size) atol(tmp.c_str());
  }
}
...
{                 
  ...
  pChunkData = data; 
  Size chunkNum = 0; 
  Size totalChunks = ((len - 1) / fixedChunkSize) + 1;

  GMessage * chunkData = NULL;

   // chop fixedChunkSize bytes and make a chunk from it
  while( pChunkData < data + len - fixedChunkSize ) 
  {
    chunk = new GMessage();
    ...
    chunkData = new GMessage();
     ...
     // Data
    chunkData->SetAsBytes(GM_DATA, pChunkData, fixedChunkSize);
  ...
  if(pChunkData < data + len)
  {
    ...
    chunkData = new GMessage();
    char * rest = new char[fixedChunkSize];
    memset(rest, 'w', fixedChunkSize);
    memcpy(rest, pChunkData, len - (pChunkData - data));
    chunkData->SetAsBytes(GM_DATA, rest, fixedChunkSize);
    DELETE(rest);
    chunkData->SetAsBytes(GM_MESSAGE_ID, (char *)&lastMessageID, sizeof(ID));
    chunkData->SetAsBytes(GM_CHUNK_ID, (char *)&chunkNum, sizeof(Size));
    chunkData->SetAsBytes(GM_MESSAGE_LENGTH,(char *)&len, sizeof(Size));
